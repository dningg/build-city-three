...
const ARROW_KEY_SPEED = 0.2; // Adjust movement speed if needed
...
export class CameraManager {
  ...
  updateCameraPosition() {
    ...
    if (this.arrowKeys.ArrowUp) this.cameraOrigin.add(new THREE.Vector3(0, 0, ARROW_KEY_SPEED));
    if (this.arrowKeys.ArrowDown) this.cameraOrigin.add(new THREE.Vector3(0, 0, -ARROW_KEY_SPEED));
    if (this.arrowKeys.ArrowLeft) this.cameraOrigin.add(new THREE.Vector3(ARROW_KEY_SPEED, 0, 0));
    if (this.arrowKeys.ArrowRight) this.cameraOrigin.add(new THREE.Vector3(-ARROW_KEY_SPEED, 0, 0));
   ...
   * @param {KeyboardEvent} event Keyboard event arguments
   */
    onKeyDown(event) {
      if (this.arrowKeys.hasOwnProperty(event.key)) {
        this.arrowKeys[event.key] = true;
      }
    }
  
    onKeyUp(event) {
      if (this.arrowKeys.hasOwnProperty(event.key)) {
        this.arrowKeys[event.key] = false;
      }
    }
